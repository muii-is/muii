...
function drawDotSymbol(sym, alpha, invert) {
  ...
  // 엣지
  stroke(lc); strokeWeight(lw);
  edges.forEach(e=>{
    if (pts[e[0]]&&pts[e[1]])
      line(pts[e[0]].x,pts[e[0]].y,pts[e[1]].x,pts[e[1]].y);
  });
 
  // 루프 원
  let lr=clamp(r*0.17, 7, 19);
  noFill(); stroke(lc); strokeWeight(lw*0.85);
  (loops||[]).forEach(li=>{ if(pts[li]) ellipse(pts[li].x,pts[li].y,lr*2,lr*2); });
